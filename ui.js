...
function displayProjects(projectsData) {
    ...
    projectsData.forEach(({ project, project_revision, site }, index) => { 
        if (!project || !project_revision) return; 
...
function createPlaceholderProjects() {
    const placeholderData = [
        {
            title: "The Surreal Keyboard",
            description: "A dreamlike typing experience where keys morph as you type",
            stats: { views: 1243, likes: 289, comments: 42 }
        },
        {
            title: "Productivity Paradox",
            description: "A tool that helps you procrastinate more efficiently",
            stats: { views: 8543, likes: 1024, comments: 213 }
        },
        {
            title: "Whimsical Weather",
            description: "Forecasts that make you smile regardless of conditions",
            stats: { views: 3456, likes: 783, comments: 97 }
        }
    ];

    projectsGridEl.innerHTML = '<p>No real projects found. Here are some curious examples instead!</p>';

    placeholderData.forEach((project, index) => {
        const card = document.createElement('div');
        card.classList.add('project-card', 'placeholder');
        card.innerHTML = `
            <div style="text-align: none; display: contents;">
                <div class="project-thumbnail" style="background: linear-gradient(45deg, var(--secondary-color) 25%, var(--tertiary-color) 75%);"></div>
                <div class="project-info">
                    <div>
                        <h3>${project.title}</h3>
        ...
        // Add animation same as real projects
        card.style.opacity = '0';
        card.style.transform = 'translateY(20px)';
        card.style.transition = 'opacity 0.5s ease, transform 0.5s ease';
